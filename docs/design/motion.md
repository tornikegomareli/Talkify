...
The shape's bottom edge is glued to the bottom of the screen. Any animation
that moves the shape's position and overshoots will lift it off that edge and
open a gap between the shape and the screen. That gap is instantly visible,
because the illusion is that the shape is flush with the display's bottom
edge.

So: **bounce is expressed in scale anchored at the bottom, never in position.**
...
Slide arrives from outside the screen edge. Unfurl unrolls upward out of
...
Scale is always anchored `.bottom`.
