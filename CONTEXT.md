...
- The voice-reactive visual — resolved: both prototyped variants ship, user-selectable from Settings, and both replace draft text while listening. The waveform (default: the Chart Line conveyor with metallic silver treatment, plus seven alternate styles) fills the visual band; the edge glow is a three-part animation — an origin glow that blooms out of the notch housing along the shape's open silhouette on session start, breathes with the voice, and drains back when the session ends, a one-shot ripple wave rolling across the housing at session start, and a silver particle cloud drifting toward the origin while listening.
