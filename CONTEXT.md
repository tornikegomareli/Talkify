...
- A session that will shape keeps the HUD up through the shaping phase, saying which prompt it shapes with under a highlight that sweeps through the caption's own letters, and the HUD leaves early when the answer lands
- The sweep replaced a bar that filled toward the shaping timeout: the model reports no progress at all, so a bar could only time the wait rather than measure it, and a full bar on a request about to succeed reads as a failure — a sweep claims nothing beyond still running
- The shaping caption takes the live draft's place in the text band, so the shape grows nothing at release and the phase never sits under a draft still reading "Listening…"
...
- The cycling pick rides in a shoulder tag, the same capsule the language pair uses and on the opposite side: while speaking the pick is context, not the thing being read, and a band of the shape carrying it at reading size stole the glance the voice visual and the draft are there for
- Shaping adds no height to the shape at all, in either phase — nothing about the silhouette says whether shaping is on
- Both shoulder tags reserve their room from a measured text width rather than a per-character estimate, symmetrically and from the wider of the two, so a centered draft stays centered and never touches either tag
- A shaping band of the shape below the voice visual and the draft was tried and withdrawn: at reading size it dominated a session it was only annotating, and it grew the shape at release. An overlay plate centered over the visuals was withdrawn before it, because it degraded the Metal-backed visuals under it and sat on Compact's live draft
