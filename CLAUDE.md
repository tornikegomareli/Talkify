...
- **Squash by default.** A branch that iterated — prototypes, review fixes, a merge from another branch — becomes one commit on `main`.
- **Rebase only when the commits are already atomic**, meaning each one builds, passes, and is a change someone might want to land on. The test is `git bisect`: if a regression would be easier to find at one of these commits than in the whole branch, keep them.
