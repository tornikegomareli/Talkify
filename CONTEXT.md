...
- The cycling pick is drawn between the two picks the arrows would land on, the neighbours faint and smaller to either side, and the band carries no arrow glyphs at all: what Left lands on is drawn on the left, which says the same thing about something the user can read
- A press slides the names the way it moved them, so the arriving pick enters from the side the row travelled toward; Reduce Motion swaps them without the slide
